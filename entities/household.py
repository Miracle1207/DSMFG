...
class Household(BaseEntity):
    ...
    def __init__(self, entity_args):
        ...
        self.real_asset, self.real_e, self.real_income = self.get_real_data()
    ...
    # def initial_wealth_distribution(self):  #  pareto distribution
    #     asset = self.sample_real_data()
    #     return asset

    def get_real_data(self):
        df = pd.read_csv(os.path.abspath('.')+ "/agents/data/advanced_scfp2022.csv")
        asset = df['ASSET'].values
        educ = df['EDUC'].values
        income = df['INCOME'].values
        return asset,educ,income
