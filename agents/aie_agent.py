...
import torch
import numpy as np
from agents.models import aie_mlp_net, mlp_net
...
class aie_agent:
    def __init__(self, envs, args, agent_name="household"):
        ...
        if self.args.cuda:
            ...
            self.device = "cpu"
        self.net = aie_mlp_net(state_dim=self.obs_dim, num_actions=self.action_dim).to(self.device)
    ...
    def train(self, transition_dict):
        ...
        # for i in range(self.args.update_each_epoch):
            ...
            temp = torch.clamp((log_probs - old_log_probs), min=-100, max=100)
            ratio = torch.exp(temp)
